...
function DeliveryTime() {
    ...
    useEffect(() => {
        ...
        if (!isBDCDataFetched) {
            ...
            const baseUrl = "http://localhost:8088/getDeliveryOnTime?Project="+searchParams.get("ProjectSlug")+"&Sprint="+searchParams.get("SprintName");
            ...
            axios.get(baseUrl)
                .then((response) => {
                    ...
                    apidata.daily_values.map((item) => {
                        ...
                        tempSPValues.push(item.runningBVSum);
                    });
    ...
    if (!isBDCDataFetched) {
        return (
            <div> DeliveryOnTime graph loading...</div>
        )
    }
